    ...
    if (this.parentNode.getAttribute("class") == "turnout toviz") {
        g_turnout.select(".tooltip")
            .attr("transform", function() {
                return "translate(" + (mouse[0] + 10) + "," + (mouse[1] - 65) + ")"
            })
            .classed("hidden", false);
        g_bjp.select(".tooltip")
            .attr("transform", function() {
                return "translate(" + ((mouse[0] + 5) / 2 - 110) + "," + ((mouse[1] - 20) / 2 - 56) + ")";
            })
            .classed("hidden", false);
        ...
        g_turnout.select(".tooltip")
            .attr("transform", function() {
                return "translate(" + (mouse[0] * 1.7) + "," + (mouse[1] * 2 - 100) + ")"
            })
            .classed("hidden", false);
        g_bjp.select(".tooltip")
            .attr("transform", function() {
                return "translate(" + (mouse[0] - 110) + "," + (mouse[1] - 78) + ")"
            })
            .classed("hidden", false);
        g_inc.select(".tooltip")
            .attr("transform", function() {
...
function newmaps(year) {
    d3.json("map/uptopo.json", function(error, up) {
        ...
        if (document.getElementById("year_dropdown").dataset.firstRun === "true" || previous === "07") {
            ...
            g_turnout.append("text").text("VOTER TURNOUT").attr("transform", function() {
                return "translate(" + (width / 2 - 150) + ",20)"
            }).attr("class", "map_label").style("fill", party_colors["Turnout"]).attr("text-anchor", "middle");
            ...
            glist[0].append("text").text("Low").attr("transform", function() {
                return "translate(" + ((width / 2 - 185)) + "," + (50) + ")";
            }).attr("class", "map_label_small").attr("text-anchor", "middle");
            ...
            glist[0].append("text").text("High").attr("transform", function() {
                return "translate(" + ((width / 2 - 115)) + "," + (50) + ")";
            }).attr("class", "map_label_small").attr("text-anchor", "middle");
            ...
            for (var i = 1; i < glist.length; i++) {
                ...
                glist[i].append("text").text(reflist[i]).attr("transform", function() {
                    return "translate(" + ((width / 4 - 55)) + "," + (20) + ")";
                }).attr("class", "map_label").attr("text-anchor", "middle").style("fill", party_colors[reflist[i]]);

                glist[i].append("text").text("High Loss").attr("transform", function() {
                    return "translate(" + ((width / 4 - 84)) + "," + (50) + ")";
                }).attr("class", "map_label_small").attr("text-anchor", "middle");

                glist[i].append("text").text("Margin").attr("transform", function() {
                    return "translate(" + ((width / 4 - 84)) + "," + (60) + ")";
                }).attr("class", "map_label_small").attr("text-anchor", "middle");

                glist[i].append("text").text("High Victory").attr("transform", function() {
                    return "translate(" + ((width / 4 - 24)) + "," + (50) + ")";
                }).attr("class", "map_label_small").attr("text-anchor", "middle");

                glist[i].append("text").text("Margin").attr("transform", function() {
                    return "translate(" + ((width / 4 - 24)) + "," + (60) + ")";
                }).attr("class", "map_label_small").attr("text-anchor", "middle");
